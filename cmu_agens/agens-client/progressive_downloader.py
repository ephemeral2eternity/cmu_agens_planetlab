# The simple progressive downloading client code
...
# ================================================================================
def progressive_downloader(videoName, clientID, srv, srv_ip, repIdx):
	# Read MPD file
	...
	# Traces to write to google cloud
		...
		rsp_time = curTS - loadTS
                ...
                print "|---", str(int(curTS)), "---|---", str(chunkNext), "---|---", repID, "---|---", str(chunk_QoE), "---|---", str(curBuffer), "---|---", str(freezingTime), "---|---", srv, "---|---", str(rsp_time), "---|"

                client_tr[chunkNext] = dict(TS=int(curTS), Representation=repID, QoE=chunk_QoE, Buffer=curBuffer, Freezing=freezingTime, Server=srv, Response=rsp_time)
